...
export default function CustomerChatWidget({ 
    ...
    <div className="w-full h-full flex flex-col bg-white overflow-hidden rounded-2xl border border-slate-200 shadow-2xl shadow-slate-900/15">
      ...
      <div className="px-3 py-2 bg-slate-50 border-t border-slate-200 flex items-center gap-1.5 overflow-x-auto text-[11px]">
        <button
          ...
          className="px-2.5 py-1 rounded-full bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 font-medium flex items-center gap-1 shrink-0"
        >
        ...
        <button
          onClick={() => {
            ...
            addToast({
              ...
              description: 'Visitor requested human support. Chat notification dispatched to operator inbox.'
            });
          }}
          className="px-2.5 py-1 rounded-full bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 font-medium flex items-center gap-1 shrink-0"
        >
      ...
      <div className="p-3 bg-white border-t border-slate-200 flex items-center gap-2 shrink-0">
        <input
          ...
          placeholder={isWidgetOffline ? 'Chatbot is currently offline...' : 'Ask Helio anything...'}
          value={inputVal}
          onChange={e => setInputVal(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && handleSend()}
          className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all disabled:opacity-50"
        />
        <button
          onClick={handleSend}
          disabled={!inputVal.trim() || isWidgetOffline || isWidgetError}
          className="p-2 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white transition-colors shrink-0 shadow-sm"
      ...
      {/* Footer Branding */}
      <div className="px-3 py-1 bg-slate-100 text-center text-[10px] text-slate-400 border-t border-slate-200/60 font-medium">
        Powered by <span className="font-bold text-slate-600">Helio AI</span> · Northstar Studio
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      {isWidgetOpen ? (
        <div className="w-[380px] h-[580px] max-w-[calc(100vw-2rem)] animate-slide-up">
          {WidgetContent}
        </div>
      ) : (
        <button
          onClick={() => setIsWidgetOpen(true)}
          className={`flex items-center gap-2.5 px-4 py-3 rounded-full text-white shadow-xl hover:shadow-2xl transition-all duration-200 hover:scale-105 active:scale-95 ${
            chatbot.launcherStyle === 'icon' ? 'p-3.5 rounded-full' : ''
          }`}
          style={{ backgroundColor: chatbot.themeColor }}
          aria-label="Open Chat Widget"
        >
          <MessageSquare className="w-5 h-5 text-white shrink-0" />
          {chatbot.launcherStyle !== 'icon' && (
            <span className="text-xs font-bold tracking-tight pr-1">Chat with Helio</span>
          )}
