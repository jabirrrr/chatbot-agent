...
| **REQ-BOT-04** | Appearance Customization & Theme Config | P0 | M3 | `app.api.widget.config`, `frontend.components`| `TEST-BOT-004` | NOT RUN |
| **REQ-BOT-05** | JavaScript Embed Snippet Generation | P0 | M3 | `frontend.components.embed_modal`, `widget` | `TEST-BOT-005` | NOT RUN |
...
| **REQ-AI-01** | Natural Language Understanding & Streaming | P0 | M3 | `app.adapters.llm.openrouter`, SSE Endpoint | `TEST-AI-001` | NOT RUN |
| **REQ-AI-02** | Retrieval-Augmented Generation (RAG) | P0 | M3 | `app.services.rag_service`, Vector Search | `TEST-AI-002` | NOT RUN |
| **REQ-AI-03** | Conversational Lead Qualification & Capture | P0 | M3 | `app.services.conversation_service`, Tools | `TEST-AI-003` | NOT RUN |
| **REQ-AI-04** | Function / Tool Calling Infrastructure | P0 | M3 | `app.adapters.llm.tools`, Pydantic Schemas | `TEST-AI-004` | NOT RUN |
| **REQ-AI-05** | Human Agent Handoff Protocol | P0 | M5 | `app.services.handoff_service`, WebSockets | `TEST-AI-005` | NOT RUN |
| **REQ-AI-06** | Honest Fallback Behavior (No Hallucination)| P0 | M3 | `app.services.prompt_builder` | `TEST-AI-006` | NOT RUN |
| **REQ-AI-07** | LLM Abstraction (OpenRouter / OpenAI / Anthropic)| P0 | M3 | `app.adapters.llm.provider_interface` | `TEST-AI-007` | NOT RUN |
| **REQ-AI-08** | Conversation Session Memory | P0 | M3 | Redis Session Cache, `app.models.message` | `TEST-AI-008` | NOT RUN |
| **REQ-WIDGET-01**| Widget Initialization & Config Fetching | P0 | M3 | `widget/src/core/bootstrap.ts` | `TEST-WGT-001` | NOT RUN |
| **REQ-WIDGET-02**| Mobile & Desktop Responsive Design | P0 | M3 | `widget/src/ui/chat-window.ts` | `TEST-WGT-002` | NOT RUN |
| **REQ-WIDGET-03**| Messaging Interface & Typing Indicator | P0 | M3 | `widget/src/ui/message-list.ts` | `TEST-WGT-003` | NOT RUN |
| **REQ-WIDGET-04**| Inline Lead Capture Confirmation UI | P0 | M3 | `widget/src/ui/components/lead-badge.ts` | `TEST-WGT-004` | NOT RUN |
| **REQ-WIDGET-05**| Accessibility Standards (WCAG 2.1 AA) | P0 | M3 | Shadow DOM ARIA attributes & keyboard nav | `TEST-WGT-005` | NOT RUN |
| **REQ-WIDGET-06**| Error Handling & Auto-Reconnect | P0 | M3 | `widget/src/transport/sse-client.ts` | `TEST-WGT-006` | NOT RUN |
